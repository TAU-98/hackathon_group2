...
class MadmSkewde:
    ...
    def run_madm_skewde_experiment(self, num_sets=2, num_trials=10, num_practice=2, break_time=5):
        pygame.init()
        ...
        for set_idx in range(num_sets):
            ...
            # Practice
            self.visualization.show_instruction_screen(screen, f"Practice: {len(attributes)} attributes\nPress any key to start.", font, Colors.TEXT_COLOR, Colors.BG_COLOR)
            self.wait_for_key()
            practice_stimuli = stimuli_generator_madm(len(attributes), num_practice)
            for mat in practice_stimuli:
                self.run_trial(screen, font, mat, attributes, weights, Colors.TEXT_COLOR, Colors.BG_COLOR, Colors.FEEDBACK_COLOR, practice=True)
            self.visualization.show_instruction_screen(screen, "Practice complete!\nPress any key to continue.", font, Colors.TEXT_COLOR, Colors.BG_COLOR)
            self.wait_for_key()
            # Main trials
            self.visualization.show_instruction_screen(screen, f"Main trials: {len(attributes)} attributes\nPress any key to start.", font, Colors.TEXT_COLOR, Colors.BG_COLOR)
            self.wait_for_key()
            stimuli = stimuli_generator_madm(len(attributes), num_trials)
            for i, mat in enumerate(stimuli):
                res = self.run_trial(screen, font, mat, attributes, weights, Colors.TEXT_COLOR, Colors.BG_COLOR, Colors.FEEDBACK_COLOR, practice=False)
                results.append(res)
                if (i + 1) % break_time == 0 and (i + 1) != num_trials:
                    self.visualization.show_instruction_screen(screen, "Break!\nPress any key to continue.", font, Colors.TEXT_COLOR, Colors.BG_COLOR)
                    self.wait_for_key()
        # Save results
        self.save_results(results, 'madm_skewde_results.json')
        self.visualization.show_instruction_screen(screen, "Experiment complete!\nThank you!", font, Colors.TEXT_COLOR, Colors.BG_COLOR)
        pygame.time.wait(2000)
        pygame.quit()

    def run_trial(self, screen, font, mat, attributes, weights, text_color, bg_color, feedback_color, practice=False):
        self.visualization.show_candidate_table(screen, attributes, weights, mat[:, 0], mat[:, 1], font, text_color, bg_color)
        pygame.event.clear()
        ...
        while True:
            ...
            if response is not None:
                ...
                self.visualization.show_feedback(screen, msg, font, feedback_color if not correct else text_color, bg_color)
                pygame.time.wait(800)
                break
            if time.time() - start_time > timeout:
                self.visualization.show_feedback(screen, 'Too slow!', font, feedback_color, bg_color)
                pygame.time.wait(800)
                correct = False
                response = None
                rt = None
...
if __name__ == '__main__':
    madm_skerde = MadmSkewde()
    madm_skerde.run_madm_skewde_experiment()